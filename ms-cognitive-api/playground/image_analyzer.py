...
class ImageAnalyzer:
    ...
    def renderResultOnImage(self, result, img):
        ...
        if 'categories' in result:
            ...
            cv2.putText(img, categoryName, (30, 60), cv2.FONT_HERSHEY_SIMPLEX, fontScale, (0, 255, 0), fontThickness)

        if 'tags' in result:
            primary_tags = []
            for item in result['tags']: primary_tags.append(item['name'])
            primary_tags = "primary tags: " + str(primary_tags)
            cv2.putText(img, primary_tags, (30, 80), cv2.FONT_HERSHEY_SIMPLEX, fontScale, (0, 255, 0), fontThickness)
    ...
    def _connect_ms_vision_api_url(self, url):
        ...
        try:
            ...
            if result is not None:
                ...
                cv2.resizeWindow('Analyzed Image', 600, 600)
    ...
    def _connect_ms_vision_api_file(self, filepath):
        ...
        try:
            ...
            if result is not None:
                ...
                img = cv2.cvtColor(cv2.imdecode(data8uint, cv2.IMREAD_COLOR), cv2.COLOR_BGR2BGR)

                cv2.namedWindow('Analyzed Image', cv2.WINDOW_NORMAL)
                img = self.renderResultOnImage(result, img)
                cv2.resizeWindow('Analyzed Image', 600, 400)
...
# a = ImageAnalyzer()
# a.analyze_url('http://www.thephotoargus.com/wp-content/uploads/2013/01/mountain-02.jpg')
